...
set.seed(8)
rf <- randomForest(mtrain, y, ntree=170, imp=TRUE, sampsize=10000, do.trace=TRUE)
...
write_csv(submission, "res/submission_xgboost_rf.csv")
submission$Hazard <- predict_rf
write_csv(submission, "res/submission_rf.csv")
submission$Hazard <- predict_xgboost
write_csv(submission, "res/submission_xgboost.csv")
